...
form.addEventListener("submit",(event)=>{
    for(eachFormInput of allFormInputs){
        ...
        if(eachFormInput.value.trim() !=="" ){
            eachFormInput.style.border="1px solid green";
            eachFormInput.nextElementSibling.nextElementSibling.style.display="inline-block";
        }
        ...
        eachFormInput.addEventListener("input",(event)=>{
            ...
            else if(event.target.value.trim() ==="" && event.target.getAttribute("id") !== "listing[image]" ){
                event.target.style.border="1px solid red";
                event.target.nextElementSibling.style.display="inline-block";
                event.target.nextElementSibling.nextElementSibling.style.display="none";
            }
            else if(event.target.getAttribute("id") === "listing[image]" && event.target.value.trim() !==""){
                event.target.style.border="1px solid green";
            }
            else if(event.target.getAttribute("id") === "listing[image]" && event.target.value.trim() ===""){
